<template lang="html">
  <div class="userinfo-box" :class="{'screenfixed': overflow}">
    <mt-header fixed title="个人资料">
      <router-link to="/user-center" slot="left">
        <mt-button icon="back"></mt-button>
      </router-link>
      <mt-button slot="right" @click.native="saveUserInfo(1)">保存</mt-button>
    </mt-header>
    <div class="base-info">
      <div class="input-groups">
        <mt-field label="昵称" placeholder="请输入昵称" v-model="userInfo.nickName"></mt-field>
        <mt-field label="姓名" placeholder="请输入姓名" v-model="userInfo.trueName"></mt-field>
    ...
    <div class="basepeer-info">
      <mt-cell title="企业信息" id="peerStatus">
        <span style="color: #ff8800; padding-right: 10px;" v-if="userInfo.isOk==0 && userInfo.memberType==2">(待审核)</span>
        <span style="color: green; padding-right: 10px;" v-else-if="userInfo.isOk==1">(已审核)</span>
        <span style="color: orange; padding-right: 10px;" v-else-if="userInfo.isOk==-1 && userInfo.memberType==2">(待提交)</span>
        <span style="color: red; padding-right: 10px;" v-else-if="userInfo.isOk==-2 && userInfo.memberType==2">(禁止登录)</span>
        <span style="color: red; padding-right: 10px;" v-else-if="userInfo.isOk==-3 && userInfo.memberType==2">(已拒绝)</span>
        <span style="color: orange; padding-right: 10px;" v-else>(待提交)</span>
      </mt-cell>
      <mt-field label="公司名称" placeholder="请输入公司名称" :attr="{maxlength: 30}" v-model="userInfo.pubUserCpyName" v-if="userInfo.isOk==-1 || userInfo.isOk==-3"></mt-field>
      <mt-field label="公司名称" placeholder="请输入公司名称" :attr="{disabled: 'disabled'}" v-model="userInfo.pubUserCpyName" v-else></mt-field>
      <!-- <mt-field label="营业执照" placeholder="请上传您的营业执照" v-model="userInfo.pic"></mt-field> -->
      <mt-cell title="所在省市" class="require-field input-right" v-model="userInfo.addr" is-link @click.native="openAddress(2)"></mt-cell>
      <mt-field placeholder="详细地址" v-model="userInfo.area" :attr="{maxlength: 50}"></mt-field>
    </div>

    <div style="padding: 10px; background: #fff;" v-if="userInfo.isOk === -1 || userInfo.isOk === -3 ||userInfo.isOk === 0">
    ...
    <div class="show-image" v-else @click="imagePopupFn">
      <img :src="userInfo.credentialImage">
    </div>
    <div class="page-btn-groups" style="padding: 0 10px 10px; margin-top: 20px;">
      <mt-button type="primary" size="large" @click.native="saveUserInfo(2)" v-if="(userInfo.isOk === -1 || userInfo.isOk === -3 || userInfo.isOk === 0) && userInfo.memberType === 2" :disabled="btnSubCheck">提交审核</mt-button>
    </div>
    ...
    <div class="imgpop-wraper" :class="{'toggle-image': imagePopup}">
      <span class="close" @click="imagePopupFn"></span>
      <img :src="userInfo.credentialImage">
    </div>
    <!--图片弹层-->
...
export default {
  ...
  created () {
    ...
    this.$store.dispatch('addressRequest').then(end => {
      this.province = end;
      // if (this.mine.adm_province && this.mine.adm_province_id) {
      //   this.provinceCityDefault(this.mine.adm_province);
      //   this.customer.provinceCity = this.mine.adm_province;
      //   this.customer.cpyCtCityID = this.mine.adm_province_id;
      // }
  ...
  computed: {
    userInfo () {
      let userInfos = this.$store.state.user.getUser;
  ...
  methods: {
    ...
    saveUserInfo (type) {
      ...
      if(type == 1) {
        ...
        if(this.imgSrc === '' && this.userInfo.credentialImage === '') {
          this.btnSubCheck = false;
    ...
    onProgress (file, procent) {
      console.log('onProgress', procent);
    },
    onSuccess (file, ret) {},
    onError(file, err) {
      console.log("----------onError----------");
...
<style lang="css">
.userinfo-box{ padding-top: 40px; background: #f8f8f8; box-sizing: border-box;}
